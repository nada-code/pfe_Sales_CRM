...
import {
  Bell, Settings, LogOut, X, ChevronRight,
...
// Injecte les CSS variables du thème sur le conteneur racine
const cssVars = (t) => ({
...
// SETTINGS MODAL
// ════════════════════════════════════════════════════════════════════════════
function SettingsModal({ user, theme, onClose }) {
  return (
      ...
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-header">
          ...
          <div className="modal-avatar-row">
            <div className="modal-avatar">{initials(user)}</div>
            <div>
        ...
        <div className="modal-body">
          ...
          {[
            ...
            { icon: <Mail size={14} />,     label: "Adresse email", value: user.email },
            ...
            <div className="modal-info-row" key={label}>
              ...
              <div className="modal-info-content">
                ...
                <p className={`modal-info-value${highlight ? " highlight" : ""}`}>{value}</p>
              </div>
            </div>
          ))}

          <div className="modal-actions">
            <button className="modal-btn-outline" onClick={onClose}>Modifier le profil</button>
            <button className="modal-btn-filled"  onClick={onClose}>Fermer</button>
          </div>
        </div>
      ...
      {notifs.map((n) => (
        <div key={n.id} className={`notif-item${n.read ? "" : " unread"}`}>
          <div
            ...
            style={{ background: n.read ? "#cbd5e1" : "var(--role-accent)" }}
          />
          <div>
...
export default function DashboardLayout({ children }) {
  ...
  const currentLabel = nav.find((n) => location.pathname.startsWith(n.path))?.label ?? "Dashboard";
  ...
  return (
    <div className="app-shell" style={cssVars(theme)}>
      ...
      <div className="main-area">
        ...
        <header className="topbar">
          ...
          <div className="topbar-right">
            ...
            {/* Avatar chip */}
            <button className="avatar-chip" onClick={() => setShowSettings(true)}>
      ...
      {/* SETTINGS MODAL */}
      {showSettings && user && (
        <SettingsModal user={user} theme={theme} onClose={() => setShowSettings(false)} />
      )}
